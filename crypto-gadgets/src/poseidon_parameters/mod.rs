//! Precomputed poseidon parameters for fields of various pairing-friendly elliptic curves
//!
//! Parameters have been computed using the script provided by the authors of Poseidon:
//! https://extgit.isec.tugraz.at/krypto/hadeshash

#[cfg(feature = "poseidon-bn124")]
use ark_bn124 as bn124;
#[cfg(feature = "poseidon-bn183")]
use ark_bn183 as bn183;
#[cfg(feature = "poseidon-bn254")]
use ark_bn254 as bn254;
#[cfg(feature = "poseidon-ed181")]
use ark_ed181 as ed181;
#[cfg(feature = "poseidon-ed58")]
use ark_ed58 as ed58;
#[cfg(feature = "poseidon-ed61")]
use ark_ed61 as ed61;
#[cfg(feature = "poseidon-ed97")]
use ark_ed97 as ed97;
...
#[cfg(feature = "poseidon-bn124")]
pub mod parameters_bn124;
#[cfg(feature = "poseidon-bn183")]
pub mod parameters_bn183;
#[cfg(feature = "poseidon-bn254")]
pub mod parameters_bn254;
#[cfg(feature = "poseidon-ed181")]
pub mod parameters_ed181;
#[cfg(feature = "poseidon-ed58")]
pub mod parameters_ed58;
#[cfg(feature = "poseidon-ed61")]
pub mod parameters_ed61;
#[cfg(feature = "poseidon-ed97")]
pub mod parameters_ed97;
...
impl Display for PoseidonConfigError {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            PoseidonConfigError::NoConfigFound => write!(f, "No poseidon config found"),
            PoseidonConfigError::ConversionError => write!(f, "conversion error"),
        }
    }
}

impl Error for PoseidonConfigError {}

/// Choose suitable poseidon configuration
pub fn poseidon_parameters<F: PrimeField>() -> Result<PoseidonConfig<F>, PoseidonConfigError> {
    ...
    #[cfg(feature = "poseidon-bn254")]
    if modulus == bn254::Fr::MODULUS.into() {
        return parameters_bn254::poseidon_parameters();
    }
    #[cfg(feature = "poseidon-bn183")]
    if modulus == bn183::Fr::MODULUS.into() {
        return parameters_bn183::poseidon_parameters();
    }
    #[cfg(feature = "poseidon-bn124")]
    if modulus == bn124::Fr::MODULUS.into() {
        return parameters_bn124::poseidon_parameters();
    }
    #[cfg(feature = "poseidon-ed181")]
    if modulus == ed181::Fr::MODULUS.into() {
        return parameters_ed181::poseidon_parameters();
    }
    #[cfg(feature = "poseidon-ed97")]
    if modulus == ed97::Fr::MODULUS.into() {
        return parameters_ed97::poseidon_parameters();
    }
    #[cfg(feature = "poseidon-ed61")]
    if modulus == ed61::Fr::MODULUS.into() {
        return parameters_ed61::poseidon_parameters();
    }
    #[cfg(feature = "poseidon-ed58")]
    if modulus == ed58::Fr::MODULUS.into() {
        return parameters_ed58::poseidon_parameters();
    }

    Err(PoseidonConfigError::NoConfigFound)
}

#[allow(dead_code)]
fn convert_str_vec_to_fields<F: PrimeField>(
    chunk_size: usize,
    str_vec: &[&str],
) -> Result<Vec<Vec<F>>, <F as FromStr>::Err> {
    str_vec
        .chunks(chunk_size)
        .map(|chunk| {
            chunk
                .iter()
                .map(|s| F::from_str(s)) // Convert each string to a field element
                .collect::<Result<Vec<F>, _>>() // Collect results into a vector
        })
        .collect::<Result<Vec<Vec<F>>, _>>() // Collect all chunks into a vector of vectors
}
